...
function buildCard(eq) {
  ...
  wrap.addEventListener('mouseenter', () => {
    if (!isTouchDevice()) selectSlot(eq.slot);
  });
  wrap.addEventListener('mouseleave', () => {
    if (!isTouchDevice()) deselect();
  });
  wrap.addEventListener('click', (e) => {
    e.stopPropagation();
    const row   = wrap.closest('.slots-row');
    const group = wrap.closest('.cat-group');
    if (isTouchDevice() && !row.classList.contains('expanded')) {
      // primer tap en touch: expandir
      collapseAll();
      row.classList.add('expanded');
      group.classList.add('expanded');
      expandedGroup = row;
    } else {
      // desktop o grupo ya expandido: seleccionar y contraer
      selectSlot(eq.slot);
      if (isTouchDevice()) collapseAll();
    }
  });
  return wrap;
}
