...
/* ══════════════════════════════════════════════════════════════
   STATE
   selectedSlot — slot actualmente visible en el centro
   pinnedSlot   — slot fijado por click (el hover no lo reemplaza)
══════════════════════════════════════════════════════════════ */
...
function showSlot(slot) {
  ...
  /* Pin indicator */
  document.getElementById('pin-hint').style.opacity = pinnedSlot === slot ? '0' : '1';

  /* Highlight slots */
  document.querySelectorAll('.slot-wrap').forEach(w => {
    w.classList.toggle('active', parseInt(w.dataset.slot) === slot);
    w.classList.toggle('pinned-wrap', pinnedSlot !== null && parseInt(w.dataset.slot) === pinnedSlot);
  });
...
function selectSlot(slot) {
  if (pinnedSlot !== null && slot !== pinnedSlot) return; // hay pin, ignorar hover
  showSlot(slot);
}

/* ══════════════════════════════════════════════════════════════
   TOGGLE PIN — llamado desde click
══════════════════════════════════════════════════════════════ */
function togglePin(slot) {
  if (pinnedSlot === slot) {
    // desanclar
    pinnedSlot = null;
    document.getElementById('pin-hint').style.opacity = '1';
    document.querySelectorAll('.slot-wrap').forEach(w => w.classList.remove('pinned-wrap'));
  } else {
    pinnedSlot = slot;
    showSlot(slot);
  }
...
function deselect() {
  ...
  document.getElementById('pin-hint').style.opacity = '0';
...
function buildCard(eq) {
  ...
  wrap.addEventListener('click',      () => togglePin(eq.slot));
  return wrap;
}
...
function renderLeaderboard() {
  ...
  sorted.forEach((eq, i) => {
    ...
    row.addEventListener('click',      () => togglePin(eq.slot));
    col.appendChild(row);
  });
}
...
function renderDetailTable() {
  ...
  tbl.querySelectorAll('tr.data-row').forEach(r => {
    ...
    r.addEventListener('click',      () => togglePin(s));
  });
}
